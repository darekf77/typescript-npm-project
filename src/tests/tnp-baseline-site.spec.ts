...
describe(wrap.describe('Tnp Baseline Site'), async () => {
  ...
  await wrap.it(`should create baseline/site worksapces projects`,
    async (location, testName, { packageJSON, cwdChange }) => {
      ...
      const project = {
        baseline: Project.From<Project>(path.join(location, BASELINE_WORKSPACE_PROJECT_NAME)),
        site: Project.From<Project>(path.join(location, SITE_NAME)),
      };
      ...
      it(testName, async () => {
        ...
        const site = Project.From<Project>(sitePath)
