...
export class LinkedRepos extends BaseFeatureForProject<Project> {
    ...
    if (this.project.__packageJson.linkedRepos.length > 0 && !struct) {
      ...
      for (let index = 0; index < toClone.length; index++) {
        ...
        if (repo?.origin) {
          ...
          if (this.project.__packageJson.isLink) {
            // link means temp folder in release
            return;
          }
          try {
            if (this.isDestOk(destLinkedRepos)) {
              if (
                Helpers.git.checkIfthereAreSomeUncommitedChange(destLinkedRepos)
              ) {
                Helpers.warn(
                  `Ommiting update of linked repo linked-repos/${nameRepo}`.toUpperCase(),
                );
            ...
            Helpers.error(
              `Not able to clone/update repo from ${repo.origin}...

              Check your ${config.file.taon_jsonc}:
              ...
          ...
          const toLink = (repo.relativeFoldersLinks || []).filter(
            f => !(f.from === f.to && f.from === '' && f.from === ''),
          );
          ...
          for (let index = 0; index < toLink.length; index++) {
            ...
            if (element.from && element.to) {
              ...
              const filter = filterFile => {
                return (
                  path.basename(filterFile).search(this.prefixes.ORIGINAL) ===
                    -1 &&
              ...
              const fileBases = {
                from: Helpers.getRecrusiveFilesFrom(fileBasePath.from).filter(
                  filter,
                ),
                to: Helpers.getRecrusiveFilesFrom(fileBasePath.to).filter(
                  filter,
                ),
              };
              ...
              fileBases.from.forEach(sourceFilePath => {
                ...
                const foundedExistedDestPath = fileBases.to.find(
                  f1 => f1 === destFilePath,
                );

                if (foundedExistedDestPath) {
                  const contentNew = Helpers.readFile(sourceFilePath);
                  const contentExisted = Helpers.readFile(
                    foundedExistedDestPath,
                  );
              ...
              fileBases.to.forEach(sourceFilePath => {
                ...
                const foundedExistedDestPath = fileBases.from.find(
                  f1 => f1 === destFilePath,
                );
                ...
                if (foundedExistedDestPath) {
                  ...
                  Helpers.writeFile(
                    deletedFilePath,
                    '// FILE HAS BEEN DELETED FROM LINKED REPO',
                  );
                }
              });
              //#endregion
  ...
  private isDestOk(dest: string) {
    return (
      Helpers.exists(dest) &&
      Helpers.git.isInsideGitRepo(dest) &&
      Helpers.git.isGitRoot(dest)
    );
  }
}
