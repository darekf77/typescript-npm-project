export function html2md(htmlData) {
  ...
  if (tableContent !== null) {
    ...
    for (let i = 0; i < tableContent.length; i++) {
      ...
      tbodyContent[i].push(
        tableContent[i].match(/(?<=\<th>)[\s\S]*?(?=<\/th?>)/gi),
      );
      tbodyContent[i].push(
        tableContent[i].match(/(?<=\<td>)[\s\S]*?(?=<\/td?>)/gi),
      );
    }
  }
  ...
  if (preContent !== null) {
    for (let k = 0; k < preContent.length; k++) {
      ...
      preLanguage =
        preLanguage != null && preLanguage[0] != 'undefined'
          ? preLanguage[0] + '\n'
          : '\n';
