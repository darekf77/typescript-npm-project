...
export class $Test extends CommandLineFeature<{}, Project> {
  ...
  async _testSelectors(watch: boolean, debug: boolean, args: string) {
    ...
    const testType = Models.TestTypeFiredevArr.includes(possibleTest as any) ? possibleTest : void 0;
    const res = testType ? testType
      : await Helpers.consoleGui.select<Models.TestTypeFiredev>(`What do you want to test ? ${!watch
        ? '(single run '
        : '(watch mode '
