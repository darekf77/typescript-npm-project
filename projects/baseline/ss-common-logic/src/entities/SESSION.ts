...
export namespace SESSION {
  ...
  export class SESSION_REPOSITORY extends META.BASE_REPOSITORY<SESSION> {
    ...
    async getByUser(user: USER.USER, ip: string) {
      //#region @backendFunc
      const Session = await this.createQueryBuilder(META.tableNameFrom(SESSION))
        .innerJoinAndSelect(`${META.tableNameFrom(SESSION)}.user`, META.tableNameFrom(USER.USER))
    ...
    async getFrom(user: USER.USER, ip: string) {
      //#region @backendFunc
