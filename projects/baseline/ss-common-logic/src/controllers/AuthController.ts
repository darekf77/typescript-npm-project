...
export namespace AuthController {
  ...
  export class AuthController extends META.BASE_CONTROLLER {
    ...
    public get ENTITIES() {
      return {
        USER: USER.USER_META(this.connection),
        SESSION: SESSION.SESSION_META(this.connection),
        EMAIL: EMAIL.EMAIL_META(this.connection),
        EMAIL_TYPE: EMAIL_TYPE.EMAIL_TYPE_META(this.connection)
      }
    }
    ...
    public get browser() {
      ...
      return {
        ...
        async authorize(session: SESSION.SESSION) {
        ...
        async logout(session?: SESSION.SESSION) {
    ...
    @POST('/login')
    login(@BodyParam() body: IHelloJS & USER.IUSER): Response<SESSION.SESSION> {
      //#region @backendFunc
      const self = this;
      return async (req) => {
        const userIP = req.ip;
    ...
    get __authorization() {
      ...
      const self = this;
      return {
        async facebook(body: IHelloJS): Promise<USER.USER> {
        ...
        email: {
          async login(form: USER.IUSER): Promise<USER.USER> {
            function checkPassword(user: USER.USER) {
          ...
          async register(form: USER.IUSER): Promise<USER.USER> {
    ...
    get __check() {
      ...
      return {
        ...
        ifRegistration(body: USER.IUSER) {
          const {
            email,
            username,
            password,
    ...
    private async  __token(user: USER.USER, ip: string) {
      //#region @backendFunc
      if (!user || !user.id) {
    ...
    private async __createUser(formData: USER.IUSER, EmailTypeName: EMAIL_TYPE.EMAIL_TYPE_NAME) {
      //#region @backendFunc
